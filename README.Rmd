...
The signs of all coefficients mkae sense, as income increases, the log odds ratio decreases by -0.16 for every 1000 dollar increase in median income. 
...
Accuracy <- final_lasso_preds %>% 
  mutate(logodds = predict(mod1, data = final_lasso_preds),
         odds = exp(logodds),
         probability = odds/(1+odds)) %>% 
  mutate(predHighLead = case_when(probability >= 0.90~1,
                                  probability < 0.90~0)) %>% 
