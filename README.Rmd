...
Interesting that there is a population, within high lead exposure, of people living in newer houses that aren't getting tested. Within our data set, we factor the testing ratio and high lead exposure and note some interesting patterns with regards to testing ratios. We see that there seems to be several newer homes in census tracts that are in the high lead factor but are not getting tested. Among the census tracts that have high testing ratios, there is a clear pattern that older census tracts seem to be testing more often for High Lead. 
...
In the upcoming section, we will be modeling the binary outcome of whether a census tract 
# Lasso 

We use a Lasso Logistic Regression to distinguish important variables in predicting census tracts with high lead concentrations. After tuning for the best penalty, we discovered that XXX are important variables. However, it is difficult to account for spatial correlation, so we will not be interpreting the output and standard errors. Instead, we will use the important variables and fit it with a fixed effect model. 

# Fixed Effect

We decide to use a fixed effect model through the the X and Y coordinates in the centroids. We fit a constant mu for computational purposes. Because i have matern correlation coefficient, does assume isotropic.   
